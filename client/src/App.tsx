...
export function App() {
  ...
  return <div className="app">
    ...
    <main>
      ...
      {summary.data && <section className="analytics-grid bands-section">
        <article className="panel"><div className="panel-head"><div><span className="eyebrow">Permanencia</span><h3>Bandas de estancia registrada</h3></div></div><p className="context">Bandas exclusivas sobre {integer.format(summary.data.eventosCompletados)} secuencias evaluables; se conservan {integer.format(summary.data.permanenciaInvertidos)} inversiones y {integer.format(summary.data.permanenciaSinEgreso)} eventos sin egreso.</p><div className="band-grid"><div><b>&lt;12 h</b><strong>{integer.format(summary.data.permanenciaMenor12h)}</strong></div><div><b>12–&lt;24 h</b><strong>{integer.format(summary.data.permanencia12a24h)}</strong></div><div><b>24–&lt;48 h</b><strong>{integer.format(summary.data.permanencia24a48h)}</strong></div><div><b>48–72 h</b><strong>{integer.format(summary.data.permanencia48a72h)}</strong></div><div><b>&gt;72 h</b><strong>{integer.format(summary.data.permanenciaMayor72h)}</strong></div></div></article>
        <article className="panel"><div className="panel-head"><div><span className="eyebrow">Activos probables</span><h3>Antigüedad al corte</h3></div></div><p className="context">Señales acumulativas al corte de observación; no representan bandas exclusivas.</p><div className="band-grid"><div><b>&gt;24 h</b><strong>{integer.format(summary.data.activosMayor24h)}</strong></div><div><b>&gt;48 h</b><strong>{integer.format(summary.data.activosMayor48h)}</strong></div><div><b>&gt;72 h</b><strong>{integer.format(summary.data.activosMayor72h)}</strong></div><div><b>No evaluable</b><strong>{integer.format(summary.data.activosAntiguedadNoEvaluable)}</strong></div><div><b>Ingreso futuro</b><strong>{integer.format(summary.data.activosFechaIngresoFutura)}</strong></div></div></article>
      </section>}
      ...
      {triage.data && <section className="panel triage"><div className="panel-head"><div><span className="eyebrow">Triage</span><h3>Cobertura de registro</h3></div><strong>{percent(triage.data?.resumen.coberturaPct ?? null)}</strong></div>
        ...
        {((triage.data?.resumen.tiemposMayorIgual24h ?? 0) > 0 || (triage.data?.resumen.tiemposMayorIgual7d ?? 0) > 0) && <div className="notice quality">Calidad visible: {integer.format(triage.data?.resumen.tiemposMayorIgual24h ?? 0)} casos ≥24 h y {integer.format(triage.data?.resumen.tiemposMayorIgual7d ?? 0)} casos ≥7 días entre Ingreso y Triage.</div>}
        ...
        <div className="band-grid"><div><b>Mismo minuto</b><strong>{integer.format(triage.data.resumen.mismoMinuto)}</strong></div><div><b>1–10 min</b><strong>{integer.format(triage.data.resumen.de1a10)}</strong></div><div><b>11–30 min</b><strong>{integer.format(triage.data.resumen.de11a30)}</strong></div><div><b>31–60 min</b><strong>{integer.format(triage.data.resumen.de31a60)}</strong></div><div><b>61–120 min</b><strong>{integer.format(triage.data.resumen.de61a120)}</strong></div><div><b>121–240 min</b><strong>{integer.format(triage.data.resumen.de121a240)}</strong></div><div><b>&gt;240 min</b><strong>{integer.format(triage.data.resumen.mayor240)}</strong></div></div>
