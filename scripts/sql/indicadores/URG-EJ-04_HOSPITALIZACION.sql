...
SELECT
  SUM(CASE WHEN fechaegr>=Fechaing THEN CONVERT(bigint,1) ELSE 0 END) AS eventos_completados,
  SUM(CASE WHEN fechaegr>=Fechaing AND destino_urg_pk=5 THEN CONVERT(bigint,1) ELSE 0 END) AS hospitalizaciones,
  CAST(100.0*SUM(CASE WHEN fechaegr>=Fechaing AND destino_urg_pk=5 THEN CONVERT(decimal(20,4),1) ELSE 0 END)
    /NULLIF(SUM(CASE WHEN fechaegr>=Fechaing THEN CONVERT(decimal(20,4),1) ELSE 0 END),0) AS decimal(9,4)) AS porcentaje_hospitalizacion
FROM EventScope;
