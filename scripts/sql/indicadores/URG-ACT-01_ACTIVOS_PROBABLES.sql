/*
  ...
  Parámetros requeridos del llamador:
    @Centro varchar(20)=NULL, @CodigoServicio int=NULL, @Corte datetime.
  Periodo: Fechaing >= @Desde AND Fechaing < DATEADD(DAY,1,@Hasta).
  El periodo no aplica; @Corte define antigüedad y reproducibilidad.
*/
WITH RawScope AS
(
...
SELECT COUNT_BIG(*) AS activos_probables,
  SUM(CASE WHEN Fechaing IS NULL THEN CONVERT(bigint,1) ELSE 0 END) AS antiguedad_no_evaluable,
  SUM(CASE WHEN Fechaing>@Corte THEN CONVERT(bigint,1) ELSE 0 END) AS fecha_ingreso_futura,
  SUM(CASE WHEN Fechaing IS NOT NULL AND Fechaing<=@Corte AND @Corte>DATEADD(HOUR,24,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_24h,
  SUM(CASE WHEN Fechaing IS NOT NULL AND Fechaing<=@Corte AND @Corte>DATEADD(HOUR,48,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_48h,
  SUM(CASE WHEN Fechaing IS NOT NULL AND Fechaing<=@Corte AND @Corte>DATEADD(HOUR,72,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_72h
FROM EventScope;
