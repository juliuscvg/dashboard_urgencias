...
SELECT COUNT_BIG(*) AS universo_total,
  SUM(CASE WHEN Fechaing IS NOT NULL AND fechaegr IS NOT NULL AND fechaegr>=Fechaing THEN CONVERT(bigint,1) ELSE 0 END) AS evaluables,
  SUM(CASE WHEN fechaegr<Fechaing THEN CONVERT(bigint,1) ELSE 0 END) AS invertidos,
  SUM(CASE WHEN fechaegr IS NULL THEN CONVERT(bigint,1) ELSE 0 END) AS sin_egreso,
  CAST(AVG(CASE WHEN fechaegr>=Fechaing THEN DATEDIFF(MINUTE,Fechaing,fechaegr)/60.0 END) AS decimal(18,2)) AS promedio_horas,
  SUM(CASE WHEN fechaegr>=Fechaing AND DATEDIFF(MINUTE,Fechaing,fechaegr)<720 THEN CONVERT(bigint,1) ELSE 0 END) AS menor_12h,
  SUM(CASE WHEN fechaegr>=DATEADD(HOUR,12,Fechaing) AND fechaegr<DATEADD(HOUR,24,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_12_a_24h,
  SUM(CASE WHEN fechaegr>=DATEADD(HOUR,24,Fechaing) AND fechaegr<DATEADD(HOUR,48,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_24_a_48h,
  SUM(CASE WHEN fechaegr>=DATEADD(HOUR,48,Fechaing) AND fechaegr<=DATEADD(HOUR,72,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_48_a_72h,
  SUM(CASE WHEN fechaegr>DATEADD(HOUR,72,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_72h
FROM EventScope;
