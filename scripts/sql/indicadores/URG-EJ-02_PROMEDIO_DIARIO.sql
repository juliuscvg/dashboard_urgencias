/*
  ...
  Parámetro adicional: @Corte datetime; hace reproducible la determinación de días completos.
*/
DECLARE @Hoy date=CONVERT(date,@Corte);
DECLARE @FinExclusivo datetime=DATEADD(DAY,1,@Hasta);
DECLARE @FinCompleto datetime=CASE WHEN @FinExclusivo<=CONVERT(datetime,@Hoy) THEN @FinExclusivo ELSE CONVERT(datetime,@Hoy) END;
DECLARE @DiasCompletos int=CASE WHEN @FinCompleto>CONVERT(datetime,@Desde) THEN DATEDIFF(DAY,@Desde,@FinCompleto) ELSE 0 END;
WITH RawScope AS
(
...
SELECT @DiasCompletos AS dias_completos,
  SUM(CASE WHEN Fechaing<@FinCompleto THEN CONVERT(bigint,1) ELSE 0 END) AS atenciones_dias_completos,
  CAST(SUM(CASE WHEN Fechaing<@FinCompleto THEN CONVERT(decimal(20,4),1) ELSE 0 END)/NULLIF(@DiasCompletos,0) AS decimal(18,2)) AS promedio_diario,
  CASE WHEN @FinExclusivo>CONVERT(datetime,@Hoy) THEN 1 ELSE 0 END AS periodo_parcial
FROM EventScope;
