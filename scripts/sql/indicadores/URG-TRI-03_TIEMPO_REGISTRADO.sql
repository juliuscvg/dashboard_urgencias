...
SELECT COUNT_BIG(*) AS universo_total,
  ...
  SUM(CASE WHEN fechatri>=Fechaing THEN CONVERT(bigint,1) ELSE 0 END) AS evaluables,
  SUM(CASE WHEN fechatri<Fechaing THEN CONVERT(bigint,1) ELSE 0 END) AS invertidos,
  CAST(AVG(CASE WHEN fechatri>=Fechaing THEN DATEDIFF(MINUTE,Fechaing,fechatri)*1.0 END) AS decimal(18,2)) AS promedio_minutos,
  SUM(CASE WHEN fechatri=Fechaing THEN CONVERT(bigint,1) ELSE 0 END) AS mismo_minuto,
  SUM(CASE WHEN fechatri>Fechaing AND fechatri<=DATEADD(MINUTE,10,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_1_a_10,
  SUM(CASE WHEN fechatri>DATEADD(MINUTE,10,Fechaing) AND fechatri<=DATEADD(MINUTE,30,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_11_a_30,
  SUM(CASE WHEN fechatri>DATEADD(MINUTE,30,Fechaing) AND fechatri<=DATEADD(MINUTE,60,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_31_a_60,
  SUM(CASE WHEN fechatri>DATEADD(MINUTE,60,Fechaing) AND fechatri<=DATEADD(MINUTE,120,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_61_a_120,
  SUM(CASE WHEN fechatri>DATEADD(MINUTE,120,Fechaing) AND fechatri<=DATEADD(MINUTE,240,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS de_121_a_240,
  SUM(CASE WHEN fechatri>DATEADD(MINUTE,240,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_240,
  SUM(CASE WHEN fechatri>=DATEADD(HOUR,24,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_igual_24h,
  SUM(CASE WHEN fechatri>=DATEADD(DAY,7,Fechaing) THEN CONVERT(bigint,1) ELSE 0 END) AS mayor_igual_7d
FROM EventScope;
